...
class m110810_162301_userTimestampFix extends CDbMigration
{
	public function safeUp()
	{
        ...
        switch ($this->dbType()) {
            case "mysql":
                    $this->addColumn(Yii::app()->getModule('user')->tableUsers,'created',"TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP");
                    $this->addColumn(Yii::app()->getModule('user')->tableUsers,'lastvisit',"TIMESTAMP NOT NULL DEFAULT '0000-00-00 00:00:00'");
                    $this->execute("UPDATE ".Yii::app()->getModule('user')->tableUsers." SET created = FROM_UNIXTIME(createtime), lastvisit = IF(lastvisit,FROM_UNIXTIME(lastvisit),'0000-00-00 00:00:00')");
                    $this->dropColumn(Yii::app()->getModule('user')->tableUsers,'createtime');
                    $this->dropColumn(Yii::app()->getModule('user')->tableUsers,'lastvisit');
                break;
            case "sqlite":
            default:
                    $this->addColumn(Yii::app()->getModule('user')->tableUsers,'created',"TIMESTAMP");
                    $this->addColumn(Yii::app()->getModule('user')->tableUsers,'lastvisit',"TIMESTAMP");
                    $this->execute("UPDATE ".Yii::app()->getModule('user')->tableUsers." SET created = datetime(createtime, 'unixepoch'), lastvisit = datetime(lastvisit, 'unixepoch')");
                    ...
                    $this->execute('INSERT INTO "'.Yii::app()->getModule('user')->tableUsers.'" SELECT "id","username","password","email","activkey","superuser","status","created","lastvisit" FROM "'.__CLASS__.'_'.Yii::app()->getModule('user')->tableUsers.'"');
	...
	public function safeDown()
	{
        ...
        switch ($this->dbType()) {
            case "mysql":
                ...
                $this->execute("UPDATE ".Yii::app()->getModule('user')->tableUsers." SET createtime = UNIX_TIMESTAMP(created), lastvisit = UNIX_TIMESTAMP(lastvisit)");
                $this->dropColumn(Yii::app()->getModule('user')->tableUsers,'created');
                $this->dropColumn(Yii::app()->getModule('user')->tableUsers,'lastvisit');
                break;
            case "sqlite":
            default:
                ...
                $this->execute("UPDATE ".Yii::app()->getModule('user')->tableUsers." SET createtime = strftime('%s',created), lastvisit = strftime('%s',lastvisit)");
                $this->execute('ALTER TABLE "'.Yii::app()->getModule('user')->tableUsers.'" RENAME TO "'.__CLASS__.'_'.Yii::app()->getModule('user')->tableUsers.'"');
                $this->createTable(Yii::app()->getModule('user')->tableUsers, array(
                    "id" => "pk",
                    "username" => "varchar(20) NOT NULL",
