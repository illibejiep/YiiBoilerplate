 ...
 * @property $alt string
 */
class Picture extends BasePicture implements IFileBased, Iillustrated
{
    ...
    {
        if ($this->id)
            return PIX_PATH. '/' .$this->id.'.'.$this->getType();
    ...
    {
        return Yii::app()->params['frontendUrl'] . PIX_URI . '/' . $this->id.'.'.$this->getType();
    }
    ...
    public function getResizeUrl($width,$height)
    {
        if ($this->id) {
            $dir = (int)($this->id/1000);
            $dir .= '/' . ($this->id -1000*(int)($this->id/1000));
            return Yii::app()->params['frontendUrl'] . PIX_URI . '/resize/' . $dir . '/' . $width.'x'.$height.'.'.$this->getType();
        }
