...
class CRUDController extends Controller
{
    ...
    public function loadModel($id = null)
	{
        ...
        $model= ActiveRecord::model($modelClassName)->findByPk($id);
        if($model===null)
    ...
    protected function _prepareModel(ActiveRecord $model)
    {
        ...
        foreach($values as $name => $value) {
            if (
                ...
                if (!is_array(ActiveRecord::model($model->relations()[$name][1])->getTableSchema()->primaryKey)) {

                    $models = array();
                    foreach (ActiveRecord::model($model->relations()[$name][1])->findAllByPk($pks) as $res)
     ...
     */
    public function getDataProvider(ActiveRecord $model)
    {
